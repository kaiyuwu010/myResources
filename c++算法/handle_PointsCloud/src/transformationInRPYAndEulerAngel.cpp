...
int main(int argc, char **argv) 
{
    ...
    if(yaml["mode"].as<int>()==0)
    {
        Eigen::Matrix4d Matrix3x3= Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>>(yaml["rotationMatrix"].as<std::vector<double>>().data());//vector::data()返回一个指向vector内存数组的指针
    }
    else if(yaml["mode"].as<int>()==1)
    {
        vector<double> rpy = yaml["rpy"].as<std::vector<double>>();
    }
    /**** 1. 旋转向量 ****/
